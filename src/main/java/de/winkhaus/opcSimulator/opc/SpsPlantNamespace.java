...
public class SpsPlantNamespace extends ManagedNamespace {

    private final Logger logger = LoggerFactory.getLogger(getClass());
    private final SubscriptionModel subscriptionModel;

    static final String NAMESPACE_URI = "urn:plant:sps";
    static final String NODEFOLDER = "data";
    static final String COUNTER = "pieces";
    static final String COUNTERREJECTAMOUNT = "rejectAmount";
    static final String MESSAGE = "messageId";
    static final String ACTIVE = "active";
    static final String MACHINEID = "machine";
    ...
    private String rejectAmountLabel;
    private String messageLabel;
    private String activeLabel;
    private String machineIdLabel;

    public SpsPlantNamespace(OpcUaServer server, MachineRepository repository, String machineId) {
        super(server, NAMESPACE_URI);
        this.machineId = machineId;
        this.repository = repository;

        this.counterLabel = String.format("%s-%s", machineId,COUNTER);
        this.rejectAmountLabel= String.format("%s-%s", machineId, COUNTERREJECTAMOUNT);
        this.messageLabel = String.format("%s-%s", machineId,MESSAGE);
        this.activeLabel = String.format("%s-%s", machineId,ACTIVE);
        this.machineIdLabel = String.format("%s-%s", machineId,MACHINEID);
    ...
    private void addDynamicNodes(UaFolderNode rootNode) {
        ...
        rootNode.addOrganizes(node(this.activeLabel, Identifiers.Boolean, AttributeDelegateChain.create(
        ...
        rootNode.addOrganizes(node(this.counterLabel, Identifiers.Double,  AttributeDelegateChain.create(
                new AttributeDelegate() {
                    @Override
                    public DataValue getValue(AttributeContext context, VariableNode node) throws UaException {
        ...
        rootNode.addOrganizes(node(this.rejectAmountLabel, Identifiers.Double,  AttributeDelegateChain.create(
                new AttributeDelegate() {
                    @Override
                    public DataValue getValue(AttributeContext context, VariableNode node) throws UaException {
                        Machine machine = repository.findByMachineId(machineId);
                        return new DataValue(new Variant(machine.getRejectAmountCounter().getPieces()));
                    }
                },
                ValueLoggingDelegate::new
        )));

        rootNode.addOrganizes(node(this.messageLabel, Identifiers.Integer,  AttributeDelegateChain.create(
                new AttributeDelegate() {
                    @Override
                    public DataValue getValue(AttributeContext context, VariableNode node) throws UaException {
        ...
        rootNode.addOrganizes(node(this.machineIdLabel, Identifiers.String,  AttributeDelegateChain.create(
                new AttributeDelegate() {
                    @Override
                    public DataValue getValue(AttributeContext context, VariableNode node) throws UaException {
